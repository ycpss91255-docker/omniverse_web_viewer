...
- **Container exits immediately at startup** — the entrypoint validates config before rendering and rejects an invalid value with an error message instead of booting a broken viewer. Rejected: a port (`SIGNALING_PORT`, `MEDIA_PORT` or `SERVE_PORT`) that is not an integer 1..65535 written without a leading zero — `80x`, `0`, `99999` and `049100` all fail — an unknown `VIEWER_UI_MODE`, or a `SIGNALING_SERVER` with illegal characters (including a stray inline `#` comment on a `host.yaml` value). Fix the reported env / `host.yaml` value.
