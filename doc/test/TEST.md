...
**142 tests** total: **44 bats** (repo-level smoke, `test/bats/smoke/devel-test/`, run in the `devel-test` stage) + **88 node** (per-package unit, `node --test`, run in the package builds and `devel-test`) + **10 Playwright** (browser e2e, `test/e2e/`: **9 tier-1** -- config dial + status states, run per-PR in the `e2e-test` extra stage -- plus **1 tier-B** visual acceptance against a real Kit producer, run nightly on a self-hosted GPU runner and on every release).
...
## test/bats/smoke/devel-test/omniverse_web_viewer_env.bats (25)
...
## apps/stream-only/test/ (63, node --test)
...
## test/e2e/ (10, Playwright + headless Chromium)
...
| `a session-start retry cannot repaint over a live picture (#75)` | REAL claims: the page dials the dead test host, so the library retries session start on its own and calls `onStart` each time. The loopback holds `#remote-video` against the library's teardown (it nulls `srcObject` at the start of every attempt -- observed, the decoded-frame counter reset to 0 within 200 ms of each `onStart`), so a picture keeps playing under those retries. Held 30 s -- two to three real attempts: the readout must stay `display: none` throughout. Non-vacuous by construction: it counts the library's own session-start log lines and asserts frames were still advancing at the end, so a hidden readout cannot pass by accident. Unfixed, it failed with `connecting to ...` from +5.9 s for a full 20.0 s, then the red `no video` for ~1.0 s |
