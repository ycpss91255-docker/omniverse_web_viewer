...
**90 tests** total: **30 bats** (repo-level smoke, `test/smoke/bats/`, run in the `devel-test` stage) + **58 node** (per-package unit, `node --test`, run in the package builds and `devel-test`) + **2 Playwright** (tier-1 browser config-dial e2e, `test/e2e/`, run in the `e2e-test` extra stage).
...
## apps/stream-only/test/ (33, node --test)
