...
- **容器启动后立即退出** — entrypoint 在渲染前会校验配置，无效值会被拒绝并打印错误信息，而不是启动一个损坏的查看器。会被拒绝的有：端口（`SIGNALING_PORT`、`MEDIA_PORT`、`SERVE_PORT`）不是「1..65535 且不带前导零」的整数 — `80x`、`0`、`99999`、`049100` 都会失败；未知的 `VIEWER_UI_MODE`；或含非法字符的 `SIGNALING_SERVER`（包括 `host.yaml` 值末尾误带的行内 `#` 注释）。请根据错误信息修正对应的环境变量 / `host.yaml` 值。
