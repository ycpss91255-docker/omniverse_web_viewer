...
- **画面在会话中途冻结，并显示 `stream stopped -- reconnecting...` 或 `stream ended -- the source is gone`**（仅 `stream-only`）— 串流开始后 Kit 生产端消失了。前者表示查看器仍在重试；后者表示已放弃重连，待 Kit 应用恢复后请刷新页面。请检查串流主机上的 Kit 进程。
