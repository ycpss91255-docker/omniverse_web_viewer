...
- **容器啟動後立即結束** — entrypoint 在渲染前會驗證設定，無效值會被拒絕並印出錯誤訊息，而非啟動一個壞掉的檢視器。會被拒絕的有：埠（`SIGNALING_PORT`、`MEDIA_PORT`、`SERVE_PORT`）不是「1..65535 且不帶前導零」的整數 — `80x`、`0`、`99999`、`049100` 都會失敗；未知的 `VIEWER_UI_MODE`；或含非法字元的 `SIGNALING_SERVER`（包含 `host.yaml` 值尾端誤帶的行內 `#` 註解）；存在但讀不到的 `host.yaml`；以及把 `public_ip` / `ui_mode` 寫在讀取範圍（`network:` / `viewer:` section）之外的 `host.yaml` — 最後這種會被拒絕而非忽略，因為忽略就等於退回 env / 預設值啟動，去連一個沒人選過的位址。請依錯誤訊息修正對應的環境變數 / `host.yaml` 值。
