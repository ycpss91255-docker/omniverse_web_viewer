...
`env_N = KEY=VALUE` 是文法，不是排版風格：`setup.sh` 只收集開頭為 `env_` 的
鍵，這個 section 裡不是這個形狀的鍵會被**靜默丟棄，且 exit 0、無任何警告**。
寫成 `SIGNALING_SERVER = <host-ip>`（這份文件有四個版本都是這樣教的）時，
`apply` 會成功、`compose.yaml` 裡什麼都沒有，viewer 靜靜地連向 `127.0.0.1`、
回應 HTTP 200，而畫面永遠不會出現。
...
這個 job 同時也是發布關卡：任何版本都必須在該 commit 上通過它才能發布，沒有 override、沒有 `continue-on-error`，因此 GPU runner 不可用時發布會被擋下而不是放行。這套接線本身也在測試涵蓋範圍內 — `release_gate_workflow.bats` 會讀取 `.github/workflows/main.yaml`，只要關卡的 `if:` / `needs:` 結構被拿掉就會失敗；而且它會對「只刪掉該項性質」的工作流程副本再跑一次檢查器，證明每條斷言真的會紅。 這個檢查涵蓋 job 層級、step 層級，以及從檔案本身推導出來的「會發布東西的 job」集合，而不是一份寫死的名單；對兩個「工作內容就是單一 driver」的 job（`tier-b-visual-e2e`、`verify-tag-shape`），它會斷言那道指令原封不動、無條件執行，且該 job 內不得有第二個以其他寫法呼叫同一支腳本的 step — 因為 gate job 就算跑起來什麼都沒做，仍然回報 `success`，而那正是 `needs:` 唯一看得到的東西。這道 pin 綁的是「這道指令真的被執行」而不是「這串字出現過」：呼叫 driver 的 step 不得帶 `shell:`、`working-directory:`、`env:`，那兩個 job 與 workflow 層級也不得有 `defaults.run` — 因為 `shell: cat {0}` 會把被 pin 的指令印出來而不是執行它，`TIER_B_*` 變數則決定它去看哪個 image，兩者都不會動到 reviewer 讀的那一行。另外 `devel-test` stage 現在複製整個 `.github/workflows/` 目錄而不是指名單一檔案，並有一個 case 斷言目錄裡到底有哪些檔案，所以「新增第二個會發布的 workflow」是一個紅掉的測試而不是一片安靜；檢查器本身仍然只讀它被指定的那一份 workflow。而且它是把 workflow 當成 YAML 解析，不是拿文字去比對 pattern：`script/ci/check_release_gates.py` 用真正的 parser 讀 `.github/workflows/main.yaml`，因為先前兩版文字比對的實作都被 review 繞過去了；運算式比對也改成不分大小寫，因為 `if: ALWAYS()` 曾經把只比對小寫的版本整個繞過去。它刻意只當一道窄關卡：它看不到什麼的完整權威清單放在 `script/ci/check_release_gates.py` 的檔頭，只留那一份，不在這裡重述。唯一值得在 README 重複的界線其實不是檢查器的事：branch protection 與 required status checks 屬於 repo 設定，這個 repo 裡沒有任何地方在檢查它們。
