...
- **コンテナが起動直後に終了する** — エントリポイントはレンダリング前に設定を検証し、無効な値はエラーメッセージとともに拒否され、壊れたビューアは起動しません。拒否されるのは: `SIGNALING_PORT` / `MEDIA_PORT` が「先頭ゼロなしの 1..65535 の整数」でない場合 — `80x`、`0`、`99999`、`049100` はすべて失敗します。`SERVE_PORT` はその整数か、ポート部分が同じ規則を満たす `tcp://<host>:<port>` エンドポイントのどちらかでなければ拒否されます（`serve -l` の残り 2 形式 `unix:` / `pipe:` は理由付きで明示的に拒否します）。ほかに、不明な `VIEWER_UI_MODE`、または不正な文字を含む `SIGNALING_SERVER`（`host.yaml` の値末尾に紛れ込んだインライン `#` コメントを含む）、存在するが読み取れない `host.yaml`、あるいは `public_ip` / `ui_mode` を読み取り対象のセクション（`network:` / `viewer:`）の外に書いた `host.yaml`。最後のケースは黙って無視せず拒否します — 無視すれば env / デフォルトのまま起動し、誰も選んでいないアドレスへ接続することになるからです。表示された環境変数 / `host.yaml` の値を修正してください。
