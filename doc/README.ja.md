...
- **コンテナが起動直後に終了する** — エントリポイントはレンダリング前に設定を検証し、無効な値はエラーメッセージとともに拒否され、壊れたビューアは起動しません。拒否されるのは: ポート（`SIGNALING_PORT` / `MEDIA_PORT` / `SERVE_PORT`）が「先頭ゼロなしの 1..65535 の整数」でない場合 — `80x`、`0`、`99999`、`049100` はすべて失敗します — 不明な `VIEWER_UI_MODE`、または不正な文字を含む `SIGNALING_SERVER`（`host.yaml` の値末尾に紛れ込んだインライン `#` コメントを含む）。表示された環境変数 / `host.yaml` の値を修正してください。
