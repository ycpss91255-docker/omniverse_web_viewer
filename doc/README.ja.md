...
このジョブはリリースゲートでもあります: リリース対象のコミットでこれが通らない限り、いかなるバージョンも公開されません。override も `continue-on-error` もないため、GPU ランナーが利用できなければリリースはブロックされます。この配線自体もテスト対象です — `release_gate_workflow.bats` は `.github/workflows/main.yaml` を読み、ゲートの `if:` / `needs:` 構造が取り除かれると失敗します。さらに、その性質だけを削除したワークフローのコピーに対してチェッカーを再実行し、各アサーションが実際に失敗しうることを証明します。 このチェックは job レベル、step レベル、およびファイルから導出した「公開しうる job」の集合を対象とします — 名前のリストではありません。作業が単一の driver である 2 つの job（`tier-b-visual-e2e`、`verify-tag-shape`）については、そのコマンドが綴りそのままで無条件に実行され、同じスクリプトを別の綴りで呼ぶ step が他に無いことを検証します — 走っただけで何もしない gate job も `success` を返し、`needs:` から見えるのはそれだけだからです。この固定は「文字列が現れること」ではなく「そのコマンドが実際に実行されること」を対象にしています: driver を呼ぶ step には `shell:` / `working-directory:` / `env:` を許さず、その 2 つの job とワークフロー階層では `defaults.run` も許しません — `shell: cat {0}` は固定されたコマンドを実行せずに出力するだけであり、`TIER_B_*` 変数は何を見るかを決めてしまうのに、どちらもレビュアーが読む行を書き換えないからです。さらに `devel-test` ステージはファイル名を指定するのではなく `.github/workflows/` ディレクトリごとコピーし、そこに何が入っているかをケースで検証します。したがって公開しうる 2 つ目のワークフローの追加は、沈黙ではなく赤いテストになります。チェッカー自体は与えられた 1 つのワークフローしか読みません。しかもワークフローをテキストとしてパターンマッチするのではなく、YAML として解析します: `script/ci/check_release_gates.py` が本物のパーサで `.github/workflows/main.yaml` を読みます — テキスト比較だった以前の 2 版がレビューで素通りされたためです。さらに式の比較は大文字小文字を区別しません — `if: ALWAYS()` が小文字だけを見る版を丸ごと素通りしたためです。これは意図的に狭いゲートであり、何が見えないかの正式な一覧は `script/ci/check_release_gates.py` のヘッダにあります。ここでは繰り返さず、その 1 か所に置いています。README に残す価値のある限界は 1 つだけで、それはチェッカーの話ではありません: branch protection と required status checks は repo の設定であり、このリポジトリのどこでもチェックしていません。
