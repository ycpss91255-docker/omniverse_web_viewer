...
このジョブはリリースゲートでもあります: リリース対象のコミットでこれが通らない限り、いかなるバージョンも公開されません。override も `continue-on-error` もないため、GPU ランナーが利用できなければリリースはブロックされます。この配線自体もテスト対象です — `release_gate_workflow.bats` は `.github/workflows/main.yaml` を読み、ゲートの `if:` / `needs:` 構造が取り除かれると失敗します。さらに、その性質だけを削除したワークフローのコピーに対してチェッカーを再実行し、各アサーションが実際に失敗しうることを証明します。 このチェックは job レベル、step レベル（gate job の作業 step がスキップされても job は `success` を返し、`needs:` から見えるのはそれだけです）、およびファイルから導出した「公開しうる job」の集合を対象とします — 名前のリストではありません。限界も明示しています: 読むのは 1 つの workflow ファイルだけで、式は既知の書き方とテキスト比較するため、reusable workflow に移されたゲートや、インラインの `run:` ブロックとして書かれたゲート作業は見えません。branch protection と required checks は repo の設定であり、どこでもチェックしていません。
